...
if (target !== 'chrome' && target !== 'firefox') {
  console.error('Target must be chrome or firefox');
  process.exit(1);
}
...
async function bundle() {
  ...
  const manifestName = target === 'chrome' ? 'manifest.chrome.json' : 'manifest.firefox.json';
  ...
  const zipPath = path.join(distDir, `randomify-${target}.zip`);
